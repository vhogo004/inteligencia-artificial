...
def mostraTab(tab):
    ...
    print()
# DFS
def DFS(tab, horizontal, vertical):
    ...
    pilha = []
    # Adiciona o estado na pilha e marca como visitado no set
    pilha.append(QC8(tab,horizontal,vertical,0))
    visitado.add(tuple(map(tuple,tab)))
 
    while pilha:
        # Tira o valor do topo da pilha
        atual = pilha.pop()
        ...
        if objetivo(atual.tab):          
            ...
            for i in range (4):
                ...
                if valida(horizontal2, vertical2):
                    ...
                    # Caso o estado atual agora não tenha sido visitado, adiciona a pilha
                    if tabi not in visitado:
                        
                        visitado.add(tabi)
                        pilha.append(QC8(tab2, horizontal2, vertical2, atual.profundidade + 1))
    # Caso de erro
...
for i in lista1:
    ...
    inicio = time()
    passos, prof = DFS(instancia, horizontal, vertical)
    fim = time()
